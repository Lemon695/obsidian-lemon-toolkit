...
- **打开命令面板** - 打开专属的命令面板，仅显示 Lemon Toolkit 的命令。命令可按最近使用或最多使用排序，固定的命令始终显示在最上面。支持模糊搜索和使用统计
...
- **显示插件使用统计** - 查看基于命令使用情况的插件使用频率：

**功能特性**：
- **使用排行**：按总命令使用次数排序插件
- **详细指标**：显示每个插件的命令数量和总使用次数
- **快速访问**：点击任意插件可打开其设置页面
- **智能聚合**：自动从命令统计中提取插件数据
...
- **打开插件管理器** - 在一个界面中管理所有插件，提供高级功能：

**功能特性**：
- **启用/禁用插件**：一键切换插件开关
- **插件信息**：查看名称、版本、作者、描述和更新时间
- **使用统计**：查看每个插件的命令使用频率
- **智能筛选**：按启用/禁用状态筛选
- **搜索**：通过名称、ID 或作者快速查找插件
- **多种排序选项**：
  - 使用次数：按最常用插件排序
  - 名称：按字母顺序排序
  - 最近使用：按最后命令使用时间排序
  - 最近更新：按插件更新时间排序
- **快速操作**：直接从管理器打开插件设置
- **自动刷新**：每 30 分钟自动扫描插件元数据
- **手动刷新**：点击刷新按钮立即更新插件信息

**显示信息**：
- 插件排名（按使用次数排序时前 3 名高亮显示）
- 插件名称和描述
- 版本、作者和最后更新时间
- 启用/禁用状态徽章
- 使用统计（命令数量和总使用次数）
...
**插件元数据追踪**：
- 自动从文件系统追踪插件更新时间
- 将元数据存储在独立的 `plugin-metadata.json` 文件中
- 包含：ID、名称、版本、作者、启用状态、更新时间
...
**功能特性**：
- **智能提取**：自动提取文档中的所有标题（H1-H6）
- **代码块过滤**：排除代码块内的标题（```java、```dataviewjs 等）
- **层级显示**：通过视觉缩进显示标题层级
- **点击导航**：点击任意标题跳转到文档中的对应位置
- **一键复制**：将整个大纲以列表格式复制到剪贴板
- **空状态处理**：未找到标题时显示友好提示
...
- **排序方式** - 选择命令面板中命令的排序方式
  - 最近使用（默认）：最近使用的命令显示在最上面
  - 最多使用：按使用次数排序
  
- **时间范围**（仅在"最多使用"模式下显示）- 仅统计此时间范围内的命令使用次数
  - 最近 24 小时：仅统计最近一天内使用的命令
  - 最近 7 天：仅统计最近一周内使用的命令
  - 最近 30 天（默认）：仅统计最近一个月内使用的命令
  - 全部时间：统计安装以来的所有命令使用

- **固定命令** - 选择要固定在命令面板顶部的命令。固定的命令将始终显示在最前面，其后是排序后的命令
